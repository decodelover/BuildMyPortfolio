...
import { CompilationInputError } from "../errors/compilation-errors";
...
export interface CompilationInputProcessorParams {
  userId: string;
  builderId: string;
  planId: string;
...
export class CompilationInputProcessor {
  public static process(params: CompilationInputProcessorParams): CompilationContext {
    const {
      userId,
      builderId,
      planId,
    ...
    if (!rawInput) {
      throw new CompilationInputError("Raw wizard inputs are required for Portfolio Compilation.");
    }
    ...
    return {
      ...
      designBlueprint,
      seoBlueprint,
      qualityReport: qualityReport as QualityReport
    };
  }
}
